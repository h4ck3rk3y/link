...
class TestLink(unittest.TestCase):
    ...
    def test_links_build(self):
        user_token = UserTokens(
            stackoverflow=UserToken(token="not really needed for stackoverflow to work"))
    ...
    def test_github_works(self):
        ...
        user_token = UserTokens(
            github=UserToken(token="this isn't really needed but needed for private results"))
    ...
    def test_both_github_and_stackoverflow(self):
        ...
        user_token = UserTokens(stackoverflow=UserToken(
            token="foobar"), github=UserToken(token="fizzbuzz"))
    ...
    def test_next_previous(self):
        ...
        user_token = UserTokens(stackoverflow=UserToken(token="foobar"))
        link = Link.builder(user_token).query("python").page_size(12)
